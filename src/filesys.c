...
action_set_date (Unit *unit, dpacket packet)
{
    ...
    a = find_aino (unit, lock, bstr (unit, name), &err);
#if defined TARGET_AMIGAOS && defined WORDS_BIGENDIAN
//rem markus if (err == 0 && SetFileDate (a->nname, (struct DateStamp *) date) == DOSFALSE)
// and add from https://github.com/salass00/e-uae/commit/f364016a910ea1bf1ef1553c9e52d33b9aa42cb8
//      if (err == 0 && SetFileDate (a->nname, (struct DateStamp *) get_real_address(date)) == DOSFALSE)
    if (err == 0 && SetDate (a->nname, (struct DateStamp *) date) == DOSFALSE)
	err = IoErr ();
#else
